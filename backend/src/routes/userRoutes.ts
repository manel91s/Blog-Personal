...
router.post(
  ...
  body('passwordConfirmation').custom((value, { req }) => {
    if (value !== req.body.password) {
      throw new Error('La contrase帽a de confirmaci贸n es incorrecta');
    }
    return true;
  }),
  ],
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      return res.status(200).json({ msg: 'Se ha enviado un email de confirmaci贸n' });
    } catch (e) {
...
router.patch(
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      if (!user) {
        const error = new Error('No se pudo restablecer la contrase帽a');
        return res.status(400).json({ msg: error.message });
      }
      ...
      return res.status(200).json({ msg: 'Se ha cambiado la contrase馻 correctamente' });
    } catch (e) {
      console.log(e);
      return res.status(400).json({ msg: e });
    }
